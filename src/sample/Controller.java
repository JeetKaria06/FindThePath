...
public class Controller {
    ...
    @FXML
    private Circle RED;
    private Color oricolor=Color.web("#ffffffff");
    private Color oricolorgreen=Color.web("0x27ae60ff");
    private Color oricolorwall=Color.web("0x7f8c8dff");         //oricolorwall = 636e72
    private Color oricolorred=Color.web("0xe74c3cff");
    ...
    private Object GBclk;
    private Object RBclk;
    private Object WBclk;

    public int GreenPointOn=0;
    public int WallPointOn=0;
    public int RedPointOn=0;
    @FXML
    private Label STARTPOINTLAB;
    @FXML
    private Label WALLLAB;
    @FXML
    private Label REDLAB;
    @FXML
    private Circle WALL;
    @FXML
    ...
    public int EndEnbl = 0;

    void checkButton(int RedPointOn,int WallPointOn,int GreenPointOn){
        if(WallPointOn==0){
            WALL.setStyle("-fx-fill:  #7f8c8d;");
            WallPointOn=0;
        }
        if(RedPointOn==0){
            RED.setStyle("-fx-fill:#e74c3c;");
            RedPointOn=0;
        }
        if(GreenPointOn==0){
            GREEN.setStyle("-fx-fill: #27ae60;");
            GreenPointOn=0;
        }
    }

    @FXML
    void RedClicked(MouseEvent event) {
        if(oricolorred.equals(RED.getFill())){
            RED.setStyle("-fx-fill:#ff6b6b;");
            RedPointOn=1;
        }
        else {
            RED.setStyle("-fx-fill:#e74c3c;");
        ...
        WallPointOn=0;
        GreenPointOn=0;
        checkButton(RedPointOn,0,0);
    }

    @FXML
    void wallclicked(MouseEvent event) {
        // System.out.println(WALL.getFill());
        if(oricolorwall.equals(WALL.getFill())){

            WALL.setStyle("-fx-fill:#bdc3c7;");
            WallPointOn=1;
        }
        else {
        ...
        RedPointOn=0;
        GreenPointOn=0;
        checkButton(0,WallPointOn,0);
    }

    @FXML
    void GreenClicked(MouseEvent event) {

        if(oricolorgreen.equals(GREEN.getFill())){
            GREEN.setStyle("-fx-fill:#3ae374;");
            GreenPointOn=1;
        }
        else {
            GREEN.setStyle("-fx-fill: #27ae60;");
        ...
        RedPointOn=0;
        WallPointOn=0;
        checkButton(0,0,GreenPointOn);
    }

    @FXML
    void ReturnId(ActionEvent event) throws FileNotFoundException, UnsupportedEncodingException, InterruptedException {

        Object bbtn = event.getSource();

        ChangeColor((Button) bbtn,RedPointOn,WallPointOn,GreenPointOn);
    }
    ...
    void changeVisitedNodeColor(int vst){
        vst++;
        (v.get("V"+vst)).setStyle("-fx-background-color:#e74c3c");
    }

    void ChangeColor(Button b,int RedPointOn,int WallPointOn,int GreenPointOn) throws FileNotFoundException, UnsupportedEncodingException, InterruptedException {

        if((b.getId()).equals("End")){
            System.out.println(b.getId());
            System.out.println("Endl clk");
            EndEnbl=1;

            int adj[][] = createConnectivityMatrix();

            PrintWriter writer = new PrintWriter("ConnectivityMatrix.txt", "UTF-8");
            for (int i = 0; i < 1275; i++) {
                for (int j = 0; j < 1275; j++) {
            ...
            DijkstraWithDS findPath = new DijkstraWithDS();
            findPath.findShortestPath(adj, startPoint, endPoint);

            System.out.println();

            int[] paths = findPath.pathArr;
            if(findPath.distanceTaken == 0){
                System.out.println("Path Doesn't Exist for such a maze.");

            } else if(findPath.distanceTaken > 0) {
                System.out.println("Shortest Distance : " + (findPath.distanceTaken));
                ...
                for (int i = paths.length-2; i > 0; i--) {
                    ...
                    Timeline timeline = new Timeline(new KeyFrame(
                            ...
                            ae -> bb.setStyle("-fx-background-color:#e74c3c;-fx-opacity: 0.5;-fx-border-width: 0.5;-fx-border-color:black;-fx-border-radius:20%;-fx-background-radius:20%;")));
                    timeline.play();
                    Thread.sleep(10);
                    //bb.setStyle("-fx-background-color:#e74c3c;-fx-opacity: 0.5;-fx-border-width: 0.5;-fx-border-color:black;-fx-border-radius:20%;-fx-background-radius:20%;");
                }
            }
        }
        if(EndEnbl==0) {
            Color color = (Color) b.getBackground().getFills().get(0).getFill();
            if (color.equals(oricolor)) {
                if (RedPointOn == 0 && WallPointOn == 0 && GreenPointOn == 0) {
                    System.out.println("in");
                    Notice.setText("Plz Select one of thise");
                } else {
                    Notice.setText("");
                    b.setStyle("-fx-border-radius:20%;-fx-background-radius: 20%; -fx-border-width: 1.5;");
                    if (RedPointOn == 1) {
                        if (RBclk == null) {
                            RBclk = b;
                        } else {
                            ((Button) RBclk).setStyle("-fx-background-color:#ffff;-fx-opacity: 0.8;-fx-border-width: 0.5;-fx-border-color:black;-fx-border-radius:20%;-fx-background-radius:20%;");
                            RBclk = b;
                        }
                        b.setStyle("-fx-background-color:#e74c3c;-fx-border-color:#ff6b6b;");
                        String st[] = b.getId().split("V");
                        endPoint = Integer.parseInt(st[1]);
                        System.out.println("End Point "+endPoint);

                    } else if (WallPointOn == 1) {
                        WBclk = b;
                        ((Button) WBclk).setStyle("-fx-background-color:#636e72;-fx-border-color:#7f8c8d;");
                        String[] st = b.getId().split("V");
                        wallArr[wallArrSize] = Integer.parseInt(st[1]);
                        wallArrSize++;
                        wallArr = Arrays.copyOf(wallArr, wallArrSize+1);
                        wallArr[wallArr.length-1] = -1;
                        for (int i = 0; i < wallArr.length; i++) {
                            System.out.print(wallArr[i]+" ");
                        }
                        System.out.println("");
                    } else if (GreenPointOn == 1) {
                        if (GBclk == null) {
                            GBclk = b;
                        } else {
                            ((Button) GBclk).setStyle("-fx-background-color:#ffff;-fx-opacity: 0.8;-fx-border-width: 0.5;-fx-border-color:black;-fx-border-radius:20%;-fx-background-radius:20%;");
                            GBclk = b;
                        }
                        b.setStyle("-fx-background-color:#3ae374;-fx-border-color:#27ae60;");
                        String st[] = b.getId().split("V");
                        startPoint = Integer.parseInt(st[1]);
                        System.out.println("Start Position "+startPoint);
                    }
                }
            } else {
                if(color.equals(Color.web("#636e72"))){
                    wallArr = Arrays.copyOf(wallArr, wallArrSize);
                    wallArrSize--;
                    System.out.println();
                    for (int i = 0; i < wallArrSize; i++) {
                        System.out.print(wallArr[i] + " ");
                ...
                b.setStyle("-fx-background-color:#ffff;-fx-opacity: 0.8;-fx-border-width: 0.5;-fx-border-color:black;-fx-border-radius:20%;-fx-background-radius:20%;");
            }
        }
    }
    ...
    @FXML
    void StoreStage(ActionEvent event) throws SQLException, ClassNotFoundException {
        int[] x = wallArr;
        int e = endPoint;
        int s = startPoint;
        String str="";
        ...
        String Quary="INSERT INTO `totalstages`( `No`,`Start`, `Wall`, `End`) VALUES (NULL,'"+s+"','"+str+"','"+e+"');";
